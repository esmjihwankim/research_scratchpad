...
APP_TIMER_DEF(i2c_sensor_tick_id);
#define TICKS APP_TIMER_TICKS(1)

APP_TIMER_DEF(i2c_ble_sampler_id);
#define FREQUENCY APP_TIMER_TICKS(25)
...
// Software timer function initializations 
...
void i2csensor_tick_handler(void *p_context)
{
  ms_ticks +=1;
...
{
  ret_code_t err_code;
  err_code = app_timer_create(&i2c_sensor_tick_id, APP_TIMER_MODE_REPEATED, i2csensor_tick_handler);
  APP_ERROR_CHECK(err_code);
...
{
  sw_timer_create();
  app_timer_start(i2c_sensor_tick_id, TICKS, NULL);
...
// ######################BLE NUS SERVICE SAMPLING APP TIMER FUNCTIONS #################
int main(void)
{
    bool erase_bonds;
    ...
    printf("\r\nUART started.\r\n");
     ...
     // Initialize I2C Slave Device Dedicated Functions
    
    i2cdev_initialize(); /** Starts twi-driver. **/
    i2cdev_enable(true);
    if(ms5611Init() == true)
    {
        printf("MS5611INIT Succeeded");    
    } 
    sw_timer_init();
    advertising_start();
